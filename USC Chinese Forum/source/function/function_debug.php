 ...
 *
 *      $Id: function_debug.php 33324 2013-05-24 07:19:04Z kamichen $
 */
...
function debugmessage($ajax = 0) {
	...
	file_put_contents(DISCUZ_ROOT.'./'.$ajaxhtml, '<?php @header(\'Content-Type: text/html; charset='.CHARSET.'\');if(empty($_GET[\'k\']) || $_GET[\'k\'] != \''.$akey.'\') { exit; } ?><style>body,table { font-size:12px; }table { width:90%;border:1px solid gray; }</style><a href="javascript:;" onclick="location.href=location.href">Refresh</a><br />');
	foreach($sqlw as $k => $v) {
		$sqlw[$k] = $k.': '.$v;
	}
	$sqlw = '('.($discuz_table ? 'discuz_table: '.$discuz_table.($sqlw ? ', ' : '') : '').($sqlw ? '<s>'.implode(', ', $sqlw).'</s>' : '').')';

	$debug = '<?php @header(\'Content-Type: text/html; charset='.CHARSET.'\');if(empty($_GET[\'k\']) || $_GET[\'k\'] != \''.$akey.'\') { exit; } ?>';

...
function plugininstall($pluginarray, $installtype = '', $available = 0) {
    ...
    $pluginarray['plugin']['way'] = intval($pluginarray['plugin']['way']);
    $pluginarray['plugin']['modules'] = serialize($pluginarray['plugin']['modules']);

    $data = array();

		if($pluginarray['catinfo']){
			$catcolumns = DB::query("SHOW COLUMNS FROM ".DB::table("common_plugin"));
			while($cattvalue = DB::fetch($catcolumns)){
				$columns[] = $cattvalue['Field'];
			}
			if(in_array('tempcat', $columns)){
				$pluginarray['plugin']['tempcat'] = serialize(array('catcode' => $pluginarray['catinfo']['catcode'], 'catname' => $pluginarray['catinfo']['catname'], 'catstatus' => $pluginarray['catinfo']['catstatus']));
			}
		}

    foreach ($pluginarray['plugin'] as $key => $val) {
        if (in_array($key, array('directory', 'datatables'))) {
            continue;
    ...
    if($pluginarray['catinfo'])check_cat($pluginid, $pluginarray['catinfo']['catcode'], $pluginarray['catinfo']['catname'], $pluginarray['catinfo']['catstatus']);
...
function check_cat($pluginid, $catcode, $catname, $catstatus) {
    if (empty($catcode))
        return;
    $cat = C::t('common_plugincat')->fetch_by_catcode($catcode);
    if (!$cat) {
        $cat['catid'] = C::t('common_plugincat')->insert(array('catcode' => $catcode, 'catname' => $catname, 'status' => $catstatus), true);
    }
    C::t('common_plugin')->update($pluginid, array('catid' => $cat['catid']));
    require_once libfile('function/cache');
    updatecache('setting');
}

function get_selectcat($catid) {
	global $cats;
	$catselectlist = '<option value="0">' . cplang('plugins_moveto') . cplang('plugins_empty') . '</option>';
	foreach ($cats as $cat) {
		$catselectlist .= '<option value="' . $cat['catid'] . '" ' . ($catid == $cat['catid'] ? 'selected="selected"' : '') . '>' . cplang('plugins_moveto') . $cat['catname'] . '</option>';
	}
	return $catselectlist;
}

function pluginlistrow($plugin) {
	global $_G, $lang, $isplugindeveloper, $submenuitem, $hookexists, $hookorder, $catid, $cats;
	return '
<li onmouseover="getMemo(this, \'' . $plugin['identifier'] . '\')">
<div id="base_' . $plugin['identifier'] . '" class="x0">
<div class="x1 cl">
<a ' . ($submenuitem ? 'href="' . ADMINSCRIPT . '?action=plugins&operation=config&do=' . $plugin['pluginid'] . '" ' : '') . 'class="avt">
<img src="' . cloudaddons_addonlogo_url($plugin['identifier']) . '" onerror="this.src=\'static/image/admincp/plugin_logo.png\';this.onerror=null" />
</a>
<p class="cl"><span class="y">' .
	(!$plugin['new'] ?
		(!$plugin['available'] ?
			'<a href="' . ADMINSCRIPT . "?action=plugins&operation=enable&pluginid=$plugin[pluginid]&formhash=" . FORMHASH . (!empty($_GET['system']) ? '&system=1' : '') . ($catid ? '&catid=' . $catid : '') . "\" class=\"bold\"><img src=\"static/image/admincp/pluginlist_off.png\" title=\"$lang[plugins_close_switch]\" /></a>" :
			'<a href="' . ADMINSCRIPT . "?action=plugins&operation=disable&pluginid=$plugin[pluginid]&formhash=" . FORMHASH . (!empty($_GET['system']) ? '&system=1' : '') . ($catid ? '&catid=' . $catid : '') . "\"><img src=\"static/image/admincp/pluginlist_on.png\" title=\"$lang[plugins_enable_switch]\" /></a>") :
			'<a href="' . ADMINSCRIPT . '?action=plugins&operation=import&dir=' . $plugin['identifier'] . ($catid ? '&catid=' . $catid : '') . '&formhash=' . FORMHASH . '" class="bold">' . $lang['plugins_install_switch'] . '</a>'
	) .
	'</span>
	<a href="' . ADMINSCRIPT . '?action=cloudaddons&id=' . $plugin['identifier'] . '.plugin" target="_blank" >' . dhtmlspecialchars($plugin['name'].' '.$plugin['version']) . '</a></p>
	<p class="cl">'.'<span class="xg1">' . $lang['author'] . ':</span> <a href="' . ADMINSCRIPT . '?action=cloudaddons&copyright=' . strip_tags($plugin['copyright']) . '" target="_blank" >' . strip_tags($plugin['copyright']) .'</a>'.'</p>'.
'</div>'.
'<div class="x2 cl">' .
	(!$plugin['new'] ?
		'<div class="x3" id="memo_' . $plugin['identifier'] . '">' .
		($plugin['pluginid'] ? '<input class="checkbox y" type="checkbox" name="check[' . $plugin['pluginid'] . ']" value="' . $plugin['pluginid'] . '" title="'.cplang('plugins_move_select').($catid ? '&nbsp;'.$cats[$catid]['catname'] : '').'">' : '').
		($hookexists !== FALSE && $plugin['available'] ? "<input title=\"$lang[hook_order]\" class=\"txt num y\" type=\"text\" id=\"displayorder_$plugin[pluginid]\" name=\"displayordernew[$plugin[pluginid]][$hookexists]\" value=\"$hookorder\" />" : '').
		'</div>'.
		($submenuitem ? '<a href="' . ADMINSCRIPT . '?action=plugins&operation=config&do=' . $plugin['pluginid'] . '">' . $lang['config'] . '</a>&nbsp;' : '') .
		(((!$plugin['modules']['system'] ? "<a href=\"" . ADMINSCRIPT . "?action=plugins&operation=delete&pluginid=$plugin[pluginid]&formhash=" . FORMHASH . ($catid ? '&catid=' . $catid : '') . "\">$lang[plugins_config_uninstall]</a>&nbsp;" : '') .
		'<a href="' . ADMINSCRIPT . '?action=cloudaddons&id=' . $plugin['identifier'] . '.plugin" target="_blank" title="' . $lang['cloudaddons_linkto'] . '">' . $lang['plugins_visit'] . '</a>&nbsp;') .
		($plugin['updateinfo'] ? '<b>' . $plugin['updateinfo'] . '</b>' : '') .
		($plugin['available'] ? '<a href="' . ADMINSCRIPT . "?action=plugins&operation=bump&pluginid=$plugin[pluginid]&formhash=" . FORMHASH . (!empty($_GET['system']) ? '&system=1' : '') . ($catid ? '&catid=' . $catid : '') . '">'. $lang['plugins_config_bump']. '</a>&nbsp;' : '') .
		($hookexists !== FALSE && $plugin['available'] ? '<a href="' . ADMINSCRIPT . "?action=plugins&operation=hook&pluginid=$plugin[pluginid]&formhash=" . FORMHASH . '">'. $lang['plugins_hooks']. '</a>&nbsp;' : '') .
		($isplugindeveloper && !$plugin['modules']['system'] ? "<a href=\"" . ADMINSCRIPT . "?action=plugins&operation=edit&pluginid=$plugin[pluginid]&formhash=" . FORMHASH . "\">$lang[plugins_editlink]</a>&nbsp;" : '')
		) : $plugin['identifier']) .
	'</div>
</div>'.
'</li>';

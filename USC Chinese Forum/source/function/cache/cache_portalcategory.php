...
function build_cache_portalcategory() {
	...
	if(!empty($domain['app']['portal'])) {
		$portaldomain = 'http'.($_G['isHTTPS'] ? 's' : '').'://'.$domain['app']['portal'].$_G['siteroot'];
	} elseif(!empty($domain['app']['default'])) {
		$portaldomain = 'http'.($_G['isHTTPS'] ? 's' : '').'://'.$domain['app']['default'].$_G['siteroot'];
	} else {
	...
	foreach($data as $key => &$value){
		...
		if($channelrootdomain && $data[$topid]['domain']){
			$url = 'http'.($_G['isHTTPS'] ? 's' : '').'://'.$data[$topid]['domain'].'.'.$channelrootdomain.'/';
			if($foldername) {

...
if($operation == 'app') {
	...
	if(!submitcheck('domainsubmit')) {
		...
		if($_G['isHTTPS']){
			showsetting('setting_domain_httpsoptimize', 'settingnew[httpsoptimize]', $_G['setting']['httpsoptimize'], 'radio');
		}else{
			showsetting('setting_domain_httpsoptimize', 'settingnew[httpsoptimize]', $_G['setting']['httpsoptimize'], 'radio', 1, 0, cplang('setting_domain_httpsoptimize_comment2', false));
		}

...
$submenu = array(
    ...
    array('cloudaddons_pack_link', 'cloudaddons" target="_blank'),
);
...
if (!$operation) {
    ...
    if (!$pcakfiles) {
        ...
        foreach ($pcakfiles as $value) {
            if ($flag < 5 && !isset($nowpackinfos[$value]) || $nowpackinfos[$value]['dateline'] + 864000 < TIMESTAMP) {
            ...
            $pack = array(
                ...
                'logo' => $nowpackinfos[$value]['logo'],
                'copyright' => $nowpackinfos[$value]['copyright'],
    ...
    showsubmit('', '', '', '<a href="' . ADMINSCRIPT . '?action=cloudaddons" target="_blank">' . cplang('cloudaddons_pack_link') . '</a>');
    showtablefooter();
    ...
    if (ispluginkey($identifier)) {
        ...
        if (!submitcheck('deletepacksubmit')) {
            ...
            showtableheader();
            if ($systemflag) {
                cpmsg('packs_delete_invalid', '', 'error', array('pack' => '<a href="' . ADMINSCRIPT . '?action=cloudaddons&id=' . $identifier . '.pack" target="_blank">' . dhtmlspecialchars($packinfos[$identifier]['name']) . '</a>'), '', false);
            } else {
...
function showpcaklist($pcaklist) {
    ...
    foreach ($pcaklist as $pack) {
        echo '<li>
				<div id="base_' . $pack['identifier'] . '" class="x0">
					<div class="x1 cl">
						<a href="' . ADMINSCRIPT . '?action=cloudaddons&id=' . $pack['identifier'] . '.pack" target="_blank" class="avt">
						<img src="' . $pack['logo'] . '" onerror="this.src=\'static/image/admincp/plugin_logo.png\';this.onerror=null" />
						</a>
						<p class="cl">' . dhtmlspecialchars($pack['name']) . '</p>
						<p class="cl">' . dgmdate($pack['dateline']) . '</p>
					</div>
					<div class="x2 cl">
						'.(!empty($pack['copyright']) ? '<p class="y">' . $lang['author'] . ': <a href="' . ADMINSCRIPT . '?action=cloudaddons&copyright=' . strip_tags($pack['copyright']) . '" target="_blank" >' . strip_tags($pack['copyright']) .'</a>'.'</p>':'').'
    ...
    echo "</ul><script>function getMemo(obj, id) { var baseobj = $('base_' + id);var memoobj = $('memo_' + id);baseobj.className = 'x0 over';memoobj.style.display = 'x0';if(!obj.onmouseout) {obj.onmouseout = function () {baseobj.className = '';memoobj.style.display = 'none';}}}</script>";
}

function get_packinfo($pack) {
    global $_G;
    $retirn = array('name' => $pack, 'dateline' => TIMESTAMP, 'logo' => 'static/image/admincp/plugin_logo.png');
    if (ispluginkey($pack)) {
        $url = CLOUDADDONS_WEBSITE_URL . '/?@' . $pack . '.pack';
        $content = dfsockopen($url, 0, '', '', FALSE, '', 5);
        $content = diconv($content, 'GBK');
        preg_match('/<h1>([^<]*)<span class="xs2">/iUs', $content, $matche);
        if ($matche[1]) {
            $retirn['name'] = trim($matche[1]);
        }
        preg_match('/<dl class="gst">([^\/]*)<\/dt>/iUs', $content, $matche);
        if ($matche[1]) {
            $retirn['copyright'] = dhtmlspecialchars(trim(strip_tags($matche[1])));
        }
        $retirn['logo'] = cloudaddons_addonlogo_url($pack, 'pack');
    }
    return $retirn;
}